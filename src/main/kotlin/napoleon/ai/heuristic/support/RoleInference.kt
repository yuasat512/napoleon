...
class RoleInference(
    ...
    // ナポレオン視点では自分の手札やキティから副官カードの所在を引き当てられれば副官兼任確定。
    // それ以外は仮説整合性チェックで絞り込む。連合軍視点では副官兼任も仮説に含める。
    ...
    fun adjutantCandidates(): List<Int> {
        ...
        // 連合軍視点では napId (副官兼任) も候補に残す。ナポレオン視点では napId == viewer.id の
        // ため自動的に除外され、副官兼任確定は上で処理済み。
    ...
    private fun hypothesisConsistent(candAdjId: Int): Boolean {
        ...
        // 副官兼任 (candAdjId == napId) では副官カードがキティに裏向きで戻されている可能性があるため原則スキップ。
        // ただし絵札の副官カードはキティ送りなら即公開されるので、未公開なら手札にあると確定でき副官兼任にも適用する。
        // ジョーカー等の非絵札は裏向きで戻せて公開されない (副官兼任の可能性が残る) ため、スキップを維持する。
        val adj = context.adjutantCard
