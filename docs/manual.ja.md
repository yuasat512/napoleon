...
縦並びのボタンダイアログで「パス」または「(スート) N枚」を選ぶ。各スートのボタンには、そのスートで出せる最小の枚数が表示され、最大宣言 (スペード20枚) を超えるスートのボタンは無効化される。
...
総合欄には全体の勝率を表示し、**通算** (メニュー → 成績表) では平均得点を、**セッション終了時**は今回得点と順位を併記する。
